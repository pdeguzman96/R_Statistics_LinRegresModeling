...
```{r score-dist}
q3 <- ggplot(evals,aes(score))+geom_histogram(bins=14)
q3 + labs(title="Distribution of Score")
summary(evals$score)
nrow(evals %>% filter(score<3))
```
...
glimpse(evals)
# age & bty_avg
p1 <- ggplot(evals,aes(age,bty_avg))+geom_point()
p1 + labs(Title = "Age vs Average Beauty") +
  annotate("text",x=65,y=7.8,label="Weak but slightly\n negative relationship")
...
p1 <- ggplot(evals,aes(bty_avg,score))+geom_jitter()
p1 + labs(title = "Average Beauty vs Score") +
  annotate("text",x=7,y=2.5,label="Weak but slightly\n positive relationship")
...
For every 1 unit increase in average beauty ratings, we expect the teacher's evaluation score to improve by about 0.067.
</div>
```{r summary-model}
summary(m_bty)
```
...
# Residual Plot
q5 <- ggplot(m_bty,aes(.fitted,.resid))+geom_point()
q5 + geom_hline(yintercept = 0)
# Distribution of Residuals
q5_2 <- ggplot(m_bty,aes(.resid))+geom_histogram(bins = 25)
q5_2
# QQ Plot
q5_3
q5_3 <- ggplot(m_bty,aes(sample=.resid))+stat_qq()
```
...
I hypothesized that ethnicity would not be a significant predictor and would have the highest p-value, but found that `cls_profs` is the most insignificant (highest p-value).
</div>
...
```
From the output above, I noticed the coefficients and p-values *slightly* changed after removing `cls_profs`. This says to me that `cls_profs` is likely collinear with other variables.
...
```{r adj-rsq-select}
m_test <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level +cls_profs+ cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
