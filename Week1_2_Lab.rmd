...
```{r runs-vs-at_bats}
q2 <- ggplot(mlb11,aes(at_bats,runs))+geom_point()
q2 + labs(title='at_bats vs runs')
...
```{r residual}
predict(m1,data.frame(at_bats=c(5579)),interval = "prediction")
...
*Yes, batting average vs runs looks very linear.*
</div>
```{r runs-vs-sth-else}
ggplot(mlb11,aes(bat_avg,runs))+geom_point()
...
compare. Does your variable seem to predict `runs` better than `at_bats`? How can you tell?

Yes, it appears that this model predicts runs better than `at_bats` because $R^2$ for `bat_avg` as the explanatory variable is 0.6561 and `at_bats` as the explanatory variable is 0.6266. This can be interpretted as `bat_avg` explaining almost 3% more variance than `at_bats`.

</div>
```{r pick-model}
bat_av <- lm(runs ~ bat_avg,mlb11)
summary(bat_av)
```
```{r}
# Plotting residuals against fitted values
ggplot(bat_av, aes(.fitted,.resid))+geom_point()+geom_hline(yintercept = 0)
# Plotting Distribution of residuals
ggplot(bat_av,aes(.resid))+geom_histogram()
# Plotting normal-probability plot / quartile-quartile plot
ggplot(bat_av,aes(sample = .resid))+stat_qq()
```
...
```{r runs-vs-others}
hits <- lm(runs ~ hits,mlb11)
summary(hits)

wins <- lm(runs ~ wins,mlb11)
summary(wins)

```

Simply by comparing each of the calculated $R^2$ values for each fitted linear model using each of these variables, `bat_avg` appears to be the best predictor since it explains the most variance (highest $R^2$).
...
```{r runs-vs-new-vars}
new_obs <- lm(runs~new_obs,mlb11)
summary(new_obs)

new_slug <- lm(runs~new_slug,mlb11)
summary(new_slug)

new_onbase <- lm(runs~new_onbase,mlb11)
summary(new_onbase)
```
...
```{r diag}
ggplot(data=new_obs,aes(.fitted,.resid))+geom_point()+geom_hline(yintercept = 0)
ggplot(data=new_obs,aes(.resid))+geom_histogram(bins=10)
ggplot(data=new_obs,aes(sample=.resid))+stat_qq()
